...
namespace OracleExecReader
{
    ...
    /// 
    /// Oracle.ManagedDataAccess.dll  - is the only dll needed.  No oracle client. 
    /// </summary> 
    class Program
    {
        ...
        private static void help()
        {
            ...
            Console.WriteLine("---------------------------------------------------------------------------------------------------");
            Console.WriteLine("OracleExecReader - A simple command line utility to pull data from an Oracle Database. ");
            ...
            Console.WriteLine("Syntax: OracleExecReader.exe <ConnectionInfo> <SQLQuery> [OutputFile]");
            Console.WriteLine("Examples:");
            Console.WriteLine("  Console output all params as  inline  : OracleExecReader.exe \"conn string\" \"select * from.. \"");
            Console.WriteLine("  Connect string in file, inline query  : OracleExecReader.exe ConnStr.txt \"select * from product \"");
            Console.WriteLine("  Console output with params in files   : OracleExecReader.exe ConnStr.txt Query.sql");
            Console.WriteLine("  Tab delimited  if out file is .txt    : OracleExecReader.exe ConnStr.txt Query.sql ouput.txt");
            Console.WriteLine("  Comma delimited if out file is .csv   : OracleExecReader.exe ConnStr.txt Query.sql ouput.csv");
            Console.WriteLine("  Html table formatted if  .htm or .html: OracleExecReader.exe ConnStr.txt Query.sql ouput.htm");
            Console.WriteLine("  Html table with banner def h2 format  : OracleExecReader.exe ConnStr.txt Query.sql H_Table_A.htm");
            Console.WriteLine("  Html with banner size format (H1..H5) : OracleExecReader.exe ConnStr.txt Query.sql H1_Table_A.htm");
        ...
        private static string GetFieldAt(OracleDataReader dr, int pos, string delimiter)
        {
            ...
            try
            {
                ...
                if (t.Name == "DateTime")
                {
                    ...
                    s = string.Format("{0}", dr.GetOracleDecimal(pos));
                ...
                {   // csv will have double quotes on strings. 
                    if (delimiter == ",")
                        s = s = string.Format("\"{0}\"", dr.GetOracleString(pos));
                    else
                        s = s = string.Format("{0}", dr.GetOracleString(pos));
                }
                else
